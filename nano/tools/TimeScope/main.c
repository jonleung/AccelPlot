...
printf( "\n\nData format: Time(ms) DEBUG_0 DEBUG_1 DEBUG_2 DEBUG_3\n" ); 
if(mode==DECSIM)
{ 
  fprintf( fp,"!         DDDD\n" );
  fprintf( fp,"!         EEEE\n" );
  ...
  while (1) {
	...
	switch(state)
		...
		case 2:
			...
			last_ts=tmp_ts;
			timestamp+=ts;
			printf( "%ld %d %d %d %d   (%ld)\n",(long) ((float) timestamp/TICK_DIVISOR),!!(pin_mask&0x8), !!(pin_mask&0x4), !!(pin_mask&0x2), !!(pin_mask&0x1), timestamp-last_timestamp); 
