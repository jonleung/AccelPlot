 ...
 *         - rfRxInfo.max_length (must be defined)                                                     *
 ...
 *                                                                                                     *
 * Reception:                                                                                          *
 *     1. Call rf_rx_on() to enable packet reception (interrupt based)                                 *
 *     2. You can either poll for a packet or set a callback for whenever packets arrive:              *
 *            Polling:  Call rf_rx_packet(), it will return the number of packet's received since      *
 *                      the last call, including those handled by callback-based packet handling.      *
 *                      If the result is non-zero you may access packets directly.                     *
 *            Callback: Define a function which returns a RF_RX_INFO* and takes a RF_RX_INFO* as its   *
 *                      only argument.  You may handle your packet here, but it must be done quickly   *
 *                      as it cuts into tasks' run time.  You MUST return a RF_RX_INFO* to the the     *
 *                      RF_RX_INFO structure where the next packet will be stored (it can be the same  *
 *                      packet you just read). Pass the address of this function to                    *
 *                      set_rf_rx_callback like so: set_rf_rx_callback(&callback)                      *
 *        Note that you may create a callback to change where packets are read to but do no handling,  *
 *        so that you may develop a system for queuing received packets to be used later               *
 *     3. Call rf_rx_off() to disable packet reception (and interrupts)                                *
 *                                                                                                     *
 * FRAME FORMATS:                                                                                      *
 * Data packets:                                                                                       *
